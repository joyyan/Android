...
public class TemplateVORequester2 extends WAVORequester{
	...
	public void request(String functioncode, String pkdoc, String winid,
		...
		cachekeyforget = functioncode + pkdoc + winid + timezone + templatepk + ts;
		requestcachepkkey = functioncode + pkdoc + winid + timezone + "pk" + getKey();
		requestcachetskey = functioncode + pkdoc + winid + timezone + "ts" + getKey();
		...
		WAReqActionVO initValueActionVO = WAReqActionVO.createCommonActionVO("getCFInitData");
		...
		request(CommonServers.getServerAddress(context) + CommonServers.SERVER_SERVLET_WA, requestVO);
	}
		...
		if(null != formInit && null != templateVO) {
			...
			if(null != hItemList) {
				for(ItemInitValueVO item : hItemList) {
					...
					setTemplateVOInitValue(initItemKey,templateVO,item.getRealvalue());
				}
			}
			...
			if(null != bItemList) {
				for(BinfoInitValueVO body : bItemList) {
					...
					setTemplateVOInitBValue(childid,body.getBinitvaluelist(),templateVO);
				}
			}
		}
	...
	private void saveTemplateVO(TemplateVO templateVO) {
		 try {  
			cachekey = cachekey + templateVO.getTemplatePK() + templateVO.getTs();
		    SharedPreferences mSharedPreferences = context.getSharedPreferences(getKey(), context.MODE_PRIVATE);
			...
			editor.commit();
			if(null != templateVO) {
				PreferencesUtil.writePreference(context,requestcachepkkey, templateVO.getTemplatePK());
				PreferencesUtil.writePreference(context,requestcachetskey, templateVO.getTs());
			}
		 } catch (Exception e) {  
		    e.printStackTrace();  
		 }  
	...
	private TemplateVO getTemplateVO() {
		...
		try {  //读取已经保存的
	       SharedPreferences mSharedPreferences = context.getSharedPreferences(getKey(), Context.MODE_PRIVATE);  
	       String personBase64 = mSharedPreferences.getString(cachekeyforget, "");  //读取物料参照
	...
	private void setTemplateVOInitBValue(String childid,List<ItemInitValueVO> binitvaluelist,TemplateVO templateVO) {
		if(childid != null && binitvaluelist != null && templateVO != null) {
			...
			if(null != bList) {
				for(ElementGroupVO4Body body : bList) {
					if(body.getChilddocid() != null && body.getChilddocid().equals(childid)) {
						List<ElementDataVO> eles = body.getElements();
						for(ItemInitValueVO item : binitvaluelist) {
							String initItemKey = item.getItemkey();
							setElementInitValue(eles,initItemKey,item.getRealvalue());
						}
					}
				}
			}
	 ...
	 * @param initItemKey	为初始值中表头某一项的itemkey
	 * @param templateVO	模板vo
	 * @param initValue		初始值接口当中的某一个itemkey的初始值
	 */
	private void setTemplateVOInitValue(String initItemKey,TemplateVO templateVO,String initValue) {
	...
	/**
	 * 
	 * @param elementList	模板中的ele list
	 * @param initItemKey	初始值接口中的itemkey
	 * @param initValue		初始值当中的初始值
	 */
	private void setElementInitValue(List<ElementDataVO> elementList,String initItemKey,String initValue) {
		if(null != elementList) {
			boolean templateishave = false;		//		用于标示初始值当中的该itemkey是否存在于表单中，false 标示没有包含，true表示包含了
